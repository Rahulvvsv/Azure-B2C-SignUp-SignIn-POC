...
const authenticateForApiConnector = (req: Request, res: Response, next: Function) => {
...
app.get('/login', async (req: Request, res: Response) => {
  ...
  const persistedState = base64URLEncode(Buffer.from(JSON.stringify({ codeVerifier })));

  const encodedEmail = encodeURIComponent(email);
  const encodedTokenScope = encodeURIComponent(tokenScope);

  const b2cSignUpSignInUrl = `https://${tenantName}.b2clogin.com/${tenantName}.onmicrosoft.com/${policy}/oauth2/v2.0/authorize?client_id=${clientId}&response_type=code&redirect_uri=${redirectUri}&response_mode=form_post&scope=${encodedTokenScope}&state=${persistedState}&code_challenge=${codeChallenge}&code_challenge_method=${codeChallengeMethod}&login_hint=${encodedEmail}&prompt=login&local=signup`;
...
app.all('/auth-response', async (req: Request, res: Response) => {
  ...
  const decodedState = base64URLDecode(req.body.state || req.query.state as string);
  ...
  try {
    const tokenResponse = await axios.post(`https://${tenantName}.b2clogin.com/${tenantName}.onmicrosoft.com/${policy}/oauth2/v2.0/token`, querystring.stringify({
      client_id: clientId,
    ...
    jwt.verify(idToken, getKey, { audience: clientId, issuer: `https://${tenantName}.b2clogin.com/${tenantId}/v2.0/` }, (err: any, decoded: any) => {
      if (err) {
...
function getKey(header: any, callback: any) {
  const jwksUri = `https://${tenantName}.b2clogin.com/${tenantName}.onmicrosoft.com/${policy}/discovery/v2.0/keys`;
